...
// UpsertLedger replaces the customer's single current ledger — a fresh
...
func UpsertLedger(ctx context.Context, db *pgxpool.Pool, userID uuid.UUID, fileKey string, uploadedBy uuid.UUID) error {
	_, err := db.Exec(ctx,
		`INSERT INTO customer_ledgers (user_id, file_key, uploaded_by, updated_at)
		 VALUES ($1, $2, $3, now())
...
func GetLedgerByUserID(ctx context.Context, db *pgxpool.Pool, userID uuid.UUID) (*Ledger, error) {
	var l Ledger
	err := db.QueryRow(ctx,
		`SELECT id, user_id, file_key, uploaded_by, created_at, updated_at FROM customer_ledgers WHERE user_id = $1`,
		userID,
