...
func UpsertDailyLog(ctx context.Context, db *pgxpool.Pool, teamMemberID uuid.UUID, date, notes string) (uuid.UUID, error) {
	var id uuid.UUID
	err := db.QueryRow(ctx,
		`INSERT INTO daily_logs (team_member_id, date, notes)
		 VALUES ($1, $2, $3)
		 ON CONFLICT (team_member_id, date) DO UPDATE
		 SET notes = $3, updated_at = NOW()
		 RETURNING id`,
		teamMemberID, date, notes,
	).Scan(&id)
	return id, err
}
...
func GetDailyLogsByMemberMonth(ctx context.Context, db *pgxpool.Pool, teamMemberID uuid.UUID, year, month int) ([]DailyLog, error) {
	rows, err := db.Query(ctx,
		`SELECT id, team_member_id, date::text, notes, created_at, updated_at
		 FROM daily_logs
	...
	for rows.Next() {
		var l DailyLog
		if err := rows.Scan(&l.ID, &l.TeamMemberID, &l.Date, &l.Notes, &l.CreatedAt, &l.UpdatedAt); err != nil {
			return nil, err
		}
		logs = append(logs, l)
	}
