...
func ListHandler(db *pgxpool.Pool, rdb *cache.Client) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		list, err := models.GetAllTags(r.Context(), db)
		if err != nil {
...
func CreateHandler(db *pgxpool.Pool, rdb *cache.Client) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		id, err := models.CreateTag(r.Context(), db, name, strings.TrimSpace(req.Color))
		if err != nil {
			log.Printf("create tag error: %v", err)
			http.Error(w, "could not create tag (it may already exist)", http.StatusInternalServerError)
			return
		}
...
func UpdateHandler(db *pgxpool.Pool, rdb *cache.Client) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if err := models.UpdateTag(r.Context(), db, id, name, strings.TrimSpace(req.Color)); err != nil {
			log.Printf("update tag error: %v", err)
			http.Error(w, "could not update tag (name may already exist)", http.StatusInternalServerError)
			return
		}
...
func DeleteHandler(db *pgxpool.Pool, rdb *cache.Client) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if err := models.DeleteTag(r.Context(), db, id); err != nil {
			log.Printf("delete tag error: %v", err)
