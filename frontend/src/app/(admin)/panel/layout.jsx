...
// Flat entries are standalone links. Group entries render as a
// minimisable tab — click to expand/collapse its children — so related
// pages (Products, Users, Settings) don't each need their own row in the
// sidebar all the time.
const NAV_ITEMS = [
  { label: "Dashboard", href: "/panel", icon: ICONS.dashboard },
  {
    label: "Products",
    icon: ICONS.box,
    group: [
  ...
  { label: "Orders", href: "/panel/orders", permission: "orders_view", icon: ICONS.orders },
  { label: "Payments", href: "/panel/payments", permission: "payments_view", icon: ICONS.payments },
  { label: "Transports", href: "/panel/transports", permission: "transports_edit", icon: ICONS.truck },
  {
    label: "Users",
    icon: ICONS.partners,
    group: [
      { label: "Partners", href: "/panel/users", permission: "partners_view" },
      { label: "Employees", href: "/panel/employees", permission: "employees_view" },
  ...
  },
  {
    ...
    icon: ICONS.design,
    group: [
      { label: "Homepage", href: "/panel/homepage", permission: "homepage_view" },
      { label: "Graphics Design", href: "/panel/graphics-design", permission: "graphics_design_view" },
...
export default function AdminLayout({ children }) {
  ...
  const canSee = (item) => {
    ...
    if (item.permission) return isAdmin || userPermissions.includes(item.permission);
    return true;
  };

  // Groups collapse to their visible children only; a group with nothing
  // visible inside it (e.g. every child is admin-only and this is an
  // employee) is dropped entirely rather than showing an empty tab.
  ...
  const [openGroups, setOpenGroups] = useState(() => {
    ...
    visibleNavItems.forEach((item) => {
      if (item.group && groupIsActive(item)) initial[item.label] = true;
    });
    return initial;
  });
