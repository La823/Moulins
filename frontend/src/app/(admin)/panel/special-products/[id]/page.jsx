...
export default function EditSpecialProduct() {
  return (
    <Suspense fallback={null}>
      <EditSpecialProductInner />
    </Suspense>
  );
}

function EditSpecialProductInner() {
  const { id } = useParams();
  ...
  const handleSave = async (e) => {
    ...
    try {
      ...
      await apiFetch(`/admin/special-products/${id}`, {
        method: "PUT",
      ...
      for (let i = 0; i < newImageFiles.length; i++) {
        ...
        await apiFetch(`/admin/special-products/${id}/images`, {
          method: "POST",
          body: JSON.stringify({
            image_key: imageKey,
      ...
      for (const pdf of newPdfFiles) {
        ...
        await apiFetch(`/admin/special-products/${id}/documents`, {
          method: "POST",
        ...
        <section className="bg-white rounded-xl border border-gray-200 p-5 space-y-4">
          ...
          <div>
            ...
            <textarea
              ...
              onChange={(e) =>
                setForm({ ...form, safety_info: e.target.value })
              }
              rows={2}
              className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm text-gray-900"
            />
