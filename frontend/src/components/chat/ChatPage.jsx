...
export default function ChatPage({ basePath, heightOffsetPx = 140 }) {
  const { user } = useAuth();
  ...
  useEffect(() => {
    ...
    // bottomRef.scrollIntoView() — scrollIntoView's default alignment walks
    // up every scrollable ancestor (not just this container), which can
    // drag the whole page down to bring a bottom-of-thread sentinel into
    // "start" position. Setting scrollTop stays entirely inside this box.
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-200 overflow-hidden" style={{ height: `calc(100vh - ${heightOffsetPx}px)` }}>
      <div className="grid grid-cols-1 md:grid-cols-3 h-full min-h-0">
        {/* Conversation list */}
        <div className="md:col-span-1 border-r border-gray-200 flex flex-col h-full min-h-0">
