...
// page and re-pick the customer from a dropdown.
export default function SpecialProductsPanel({ customerId }) {
  ...
  const fetchProducts = async () => {
    try {
      setLoading(true);
      const data = await apiFetch(`/admin/special-products?customer_id=${customerId}`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      for (let i = 0; i < imageFiles.length; i++) {
        const imageKey = await uploadFileToS3(imageFiles[i], "/admin/special-products/upload-url");
      ...
      for (const pdf of pdfFiles) {
        const fileKey = await uploadFileToS3(pdf.file, "/admin/special-products/document-upload-url");
  ...
  const removePdf = (index) => setPdfFiles(pdfFiles.filter((_, i) => i !== index));

  const handleDeleteImage = async (imgId) => {
    try {
      await apiFetch(`/admin/special-products/images/${imgId}`, { method: "DELETE" });
      fetchProducts();
    } catch (err) {
      alert(err.message);
    }
  ...
  const handleDeleteDoc = async (docId) => {
    try {
      await apiFetch(`/admin/special-products/documents/${docId}`, { method: "DELETE" });
      fetchProducts();
    } catch (err) {
      alert(err.message);
    }
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-200 p-6">
      <div className="flex items-center justify-between mb-4">
        <div>
          <h3 className="text-sm font-semibold text-gray-700 uppercase tracking-wider">Special Products</h3>
          <p className="text-xs text-gray-500 mt-1">Only visible to this customer.</p>
        </div>
        <button
      ...
      {showForm && (
        <form onSubmit={handleSubmit} className="mb-6 p-4 bg-gray-50 rounded-xl border border-gray-200 space-y-4">
          ...
          <div>
            ...
            <input type="file" accept="image/*" multiple onChange={handleAddImages} className="w-full text-sm text-gray-600" />
            {imageFiles.length > 0 && (
          ...
          <div>
            ...
            <input type="file" accept=".pdf" onChange={handleAddPdf} className="w-full text-sm text-gray-600" />
            {pdfFiles.length > 0 && (
              <div className="mt-2 space-y-1">
                {pdfFiles.map((pdf, i) => (
                  <div key={i} className="flex items-center justify-between bg-white px-3 py-1.5 rounded text-sm border border-gray-200">
                    <input
                      type="text"
                      value={pdf.name}
                      onChange={(e) => {
                    ...
                    />
                    <button type="button" onClick={() => removePdf(i)} className="text-red-500 text-xs ml-2">
                      Remove
                    </button>
                  </div>
                ))}
              ...
              <div className="flex gap-4">
                  ...
                  <div className="flex gap-1.5 flex-shrink-0">
                    {p.images.map((img) => (
                      <div key={img.id} className="relative group">
                        <img src={img.image_url} alt={p.name} className="w-16 h-16 object-cover rounded-lg" />
                        <button
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-start justify-between">
                    <div>
                      ...
                      <p className="text-sm text-gray-500 mt-0.5">
                        {p.price != null ? `₹${p.price}` : "No price"} &middot; {p.stock != null ? `${p.stock} in stock` : "—"}
                      </p>
                    </div>
                    <div className="flex items-center gap-2">
                      ...
                      <button onClick={() => handleDelete(p.id)} className="text-red-600 hover:text-red-700 text-xs font-medium">
                        Delete
                      </button>
                    </div>
                  </div>
                  ...
                  {p.documents && p.documents.length > 0 && (
                    <div className="mt-2 flex flex-wrap gap-2">
                      {p.documents.map((doc) => (
                        <div key={doc.id} className="flex items-center gap-1.5 bg-gray-50 px-2 py-1 rounded text-xs">
